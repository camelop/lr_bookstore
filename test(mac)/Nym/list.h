...
public:
	struct Node{
		...
		Node(KeyType _key, long _offset, long _position, long prev, long succ)
			: offset(_offset), position(_position){
				key = _key;
				pointer[0] = prev;
				pointer[1] = succ;
		}
	...
	Node CreateBlock(ArrayNode node_to_insert, long prev, long succ){
		block_cnt++;
	...
	void Insert(KeyType key_to_insert, ValueType value_to_insert){
		...
		if (now_node.position == HEAD){
			...
			Node new_node = CreateBlock(node_to_insert, HEAD, TAIL);
		...
		if (block_array.array_cnt + 1 <= BLOCK_SIZE){
			...
			Node new_node = CreateBlock(block_array.array[block_array.array_cnt - 1], now_node.position, now_node.pointer[1]);
